...
function applyAllFiltersRealtime() {
  ...
  let rows = filteredRecords.slice(); // Create a copy of filtered records

  const missing = document.getElementById("filterMissing")?.value || "all";

  // Apply filter for "missing" fields (incomplete, funding, etc.)
  if (missing !== "all") {
    rows = rows.filter((r) => {
  ...
  displayRecords = rows; // Store filtered data in displayRecords

  // Call additional functions for toggling and AI-related processing (if necessary)
  applyCompletedRowsToggle(); // Apply "Hide Completed Rows" if toggle is active
  autoRunAI(); // Trigger AI-related logic if needed (not clear in your code)

  // If no records are found, show the "No records found" notification
  if (displayRecords.length === 0) {
    showNoDataNotification(); // Show the prompt when no records match
  } else {
    hideNoDataNotification(); // Hide the prompt if data is found
    renderTable(displayRecords); // Render the filtered data in the table
    updateSeverityStrip(); // Update severity-related information
    updateCounts(); // Update counts based on filtered data
  }
}
...
function wireUI() {
  ...
  if (closeStartBtn) {
    closeStartBtn.addEventListener("click", closeStartScreenModal);
  }
...
console.log("[StartScreen] forcing StartScreen open (dev mode)");
